...
| 年化超额收益(最终版:换手缓冲+行业中性+40 日信号平滑) | **+3.77%** |
| 信息比率 | **1.05** |
| 最大回撤(超额) | 2.1% |
| 年化单边换手率 | 3.9 倍 |
| 月度胜率 | 66.7% |
...
全部回测默认启用一字板成交约束(执行日一字涨停不可买入、一字跌停持仓冻结),成交假设从严。

总体结论:策略在测试年度实现年化超额 +3.77%、信息比率 1.05,月度胜率三分之二,超额回撤 2.1%。该结果的取得经历了明确的问题定位与解决过程:基础版本(裸 Top200)年化超额为 -6.5%,其中换手成本拖累约 6.6 个百分点;经换手缓冲、行业中性化修复至基本持平(毛超额约 +3.5%/年、成本约 3.8%/年,两相抵消)后,进一步的调仓频率实验定位出关键事实——预测信号在数周内衰减缓慢,周度重排大量消耗于排名边缘的无效换手;最终以 40 日信号平滑(排序分数取预测值滚动均值)在**保持周度调仓**的前提下将换手降至 3.9 倍,同时平滑本身对日度预测形成时间维度集成、提升了信号质量,毛超额升至约 +5.0%,扣除约 1.2% 成本后净超额转正。全部组合参数(缓冲区、平滑窗口)仅以 2022 年验证集调优,2023 年仅做最终一次评估。本报告同时完整记录了两项经实验否决的方案,以说明取舍依据。
...
季度滚动下,四折的最优验证 RankIC 随训练窗口扩张单调上升(0.049 → 0.057 → 0.072 → 0.092),表明模型持续受益于更贴近当期的训练数据,测试集 RankIC 相对提升约 18%。但策略端(最终配置口径)半年滚动反而更优:IC 的增益体现在全截面排序能力上,而 Top200 多头组合仅消费排序最右端;且季度版预测由四折模型拼接,40 日信号平滑窗口跨越折边界时的衔接噪音更大,抵消了预测端的改善。本报告以半年滚动作为主结果;预测端的单调增益支持实盘部署采用季度或更高频率的滚动更新,配合更平稳的跨折信号衔接(季度版回测明细见 `results/*_roll3.*`)。
...
- **一字板成交约束(默认启用)**:执行日一字涨停(全天最低价仍在板上)的买入目标视为不可成交,资金分摊至其余标的;一字跌停的持仓冻结继续持有。板块涨跌停幅度(10%/20%)按历史触板的扩张标志自动识别,不引入未来信息;基准为指数口径,不受该约束;
...
| + 换手缓冲(buffer=350) | -4.12% | -1.18 | 7.0% | 14.0 | 33.3% |
| + 行业中性化 | -2.58% | -0.58 | 5.0% | 20.5 | 33.3% |
| 缓冲 + 中性 | -0.29% | -0.02 | 4.5% | 12.7 | 41.7% |
| **缓冲 + 中性 + 40 日信号平滑(最终版)** | **+3.77%** | **1.05** | **2.1%** | **3.9** | **66.7%** |

基准 2023 年年化收益为 -2.30%,最终版组合为 +1.39%。各项改进的逻辑递进关系及信号平滑的设计与调优过程见 8.3、8.4 节。

![最终版净值曲线(buffer+中性+40日平滑)](results/equity_curve_smooth40.png)

对最终版做收益分解:年化单边换手 3.9 倍,双边千三对应约 1.2%/年的成本,反推毛超额约 +5.0%/年——高于未平滑版本的约 +3.5%,说明信号平滑不仅节约成本,其对日度预测的时间维度集成也提升了信号自身的质量(详见 8.4 节)。
...
**剔除最高分位(否决,如实记录)**:分组收益诊断(下图)显示预测值最高的第 10 分位未来收益反而靠后(短期大幅上涨的高波动股票存在均值回归),据此设计 skip 参数:选股时剔除预测最高的前 N 名。为避免测试集数据窥探,专门保存验证期(2022)预测进行网格搜索,skip=100 在验证集上显著最优(超额 +3.6% 对不剔除的 +1.4%,IR 1.35 对 0.37)。**但 2023 年样本外,skip=100 使缓冲加中性版超额自 -0.29% 恶化至 -5.78%**。归因有二:其一,持仓股一旦进入前 100 名即被强制卖出,换手自 12.7 回升至 20.3,并系统性卖出短线强势股;其二,基于单一年份验证集调优的组合规则,对风格切换缺乏稳健性。该实验的结论是:**组合构建层的超参数与模型参数同样存在过拟合风险,且更为隐蔽**。最终版仅保留具备独立事前逻辑的换手缓冲与行业中性化,放弃 skip 规则。
...
缓冲加中性版本(超额 -0.29%)的收益分解显示,毛超额(约 +3.5%)与交易成本(约 3.8%)基本相抵,成本是最大的单一亏损来源。为定位改进空间,首先做调仓频率敏感性实验(`--rebal-weeks` 参数):
...
月度调仓的毛超额与周度几乎相同(约 +3.7% 对 +3.5%),说明 **5 日预测信号在四周持有期内几乎不衰减**(预测排名的周间自相关较高),周度重排消耗的换手大部分发生在排名边缘、并未带来信息增益。但题目要求周度调仓,直接降频不可取。
...
N=40 为内部最优(两端均回落,非网格边界),选定后在测试集上仅评估一次,即第 7 节最终版结果:**年化超额 +3.77%、IR 1.05**,不仅优于未平滑版本,亦优于月度调仓(+1.96%)。原因在于平滑的双重作用:其一,换手自 12.7 降至 3.9 倍,成本节约约 2.6 个百分点;其二,40 日滚动均值等价于对日度预测做时间维度的集成,抑制单日预测噪声,毛超额自约 +3.5% 升至约 +5.0%。(注:本节与 8.3 节的验证集调参表在未启用一字板约束的口径下生成,该约束对结果的影响在 10bp 量级,不改变任何参数选择。)
...
**换手率对实盘成本的影响**。基础版年化单边换手约 22 倍,双边千三假设下成本拖累约 6.6%/年,曾是本策略最大的单一亏损来源;换手缓冲与信号平滑将其降至 3.9 倍(成本约 1.2%/年),已在回测中扣除,这一控制过程本身即为本项目的核心改进(8.4 节)。实盘成本还需考虑:200 只等权、单只权重 0.5% 的组合在规模放大后,冲击成本随交易金额超线性增长;规模达到数十亿量级时,VWAP 成交假设将系统性偏乐观。涨跌停导致的不可成交已在引擎中默认建模(执行日一字涨停剔除买入候选、一字跌停持仓冻结,`--no-limit-lock` 可关闭对照):2023 年一字板仅占股票日的 0.03%,关闭该约束时年化超额为 +3.89%,即从严的成交假设使结果保守约 12 个基点,结论稳健;但需注意本数据为无退市的平衡面板,天然缺失"连续一字跌停至退市"的极端场景,实盘中该约束的杀伤力显著大于回测所示。低换手(3.9 倍)在容量层面是有利属性,但进一步的稳健化仍可在组合优化中显式引入换手惩罚项。
