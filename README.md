...
make all      # 产出 results/backtest_results_smooth40.csv(年化超额 +3.8%, IR 1.05)与净值曲线
```
...
- **最终策略**:周度调仓 Top200 等权 + 换手缓冲(350)+ 行业中性化 + 40 日信号平滑,
  2023 年样本外年化超额 +3.8%、IR 1.05;组合参数仅用 2022 验证集调优(报告 8.4 节)
- **成交假设从严**:回测默认启用一字板约束(一字涨停买不进、一字跌停卖不出),
  停牌同理处理;引擎自洽性检验(--self-check)偏差为 0
